...
def check_type(output, json_dict, repo_url):
    try:
        metadata = "metadata"
        if "metadata" not in output:
            output[metadata] = {}
        if "type" in json_dict:
            ...
            elif data_model_type != extract_datamodel_from_repo_url(repo_url):
                output["metadata"]["type"] = \
                    {"warning": f"type {data_model_type} doesn't match the data model "
                                f"{extract_datamodel_from_repo_url(repo_url)}, please check it again"}
        else:
            output["metadata"]["type"] = \
...
def check_at_context(output, json_dict, repo_url):
    try:
        metadata = "metadata"
        if "metadata" not in output:
            output[metadata] = {}
        if "@context" in json_dict:
            ...
            if (KEYWORDS_FOR_CERTAIN_CHECK in repo_url) and ("@context" not in output["metadata"].keys()):
                if not (get_context_jsonld_raw(repo_url) in context):
                    output["metadata"]["@context"] = \
                        {"warning": "@context doesn't include the right context link, please check it again or ignore "
                                    "this message if it's an unpublished datamodel"}
        else:
...
metadata_check = {
    "id": lambda output, json_dict, repo_url: check_id(output, json_dict),
    "type": lambda output, json_dict, repo_url: check_type(output, json_dict, repo_url),
    "@context": lambda output, json_dict, repo_url: check_at_context(output, json_dict, repo_url),
}


def is_metadata_existed_examples(output, json_dict, repo_url, checklist=None):
...
def is_metadata_existed(output, json_dict, repo_url):
    ...
    try:
        ...
        if "$id" in json_dict:
            ...
            elif (KEYWORDS_FOR_CERTAIN_CHECK in repo_url) and (metadata_id != create_schema_json_url(repo_url)):
                output["metadata"]["$id"] = \
