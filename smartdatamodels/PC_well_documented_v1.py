...
def parse_yaml_dict(yaml_dict, data_model_repo_url, level):
    ...
    if isinstance(yaml_dict, list):
        for item in yaml_dict:
            partial_output = parse_yaml_dict(item, data_model_repo_url, level + 1)
        ...
        for prop in yaml_dict:
            ...
            if isinstance(yaml_dict[prop], dict):
                if prop in ["properties", "allOf", "oneOf", "anyOf", "items"]:
                    partial_output = parse_yaml_dict(yaml_dict[prop], data_model_repo_url, level + 1)
                    output = dict(output, **partial_output)
                    continue
                ...
                # if type is a 
                if isinstance(yaml_dict[prop], dict) and prop != "x-ngsi" and prop not in exceptions:
                    ...
                    try:
                        ...
                        output[prop] = {}
                        output[prop]["x-ngsi"] = False
                        output[prop]["x-ngsi_text"] = ("Missing NGSI type of " + str(propertyTypes)
                                                       + " in the description of the property on level "
                                                       + str(level))
                    ...
                    if prop == "type" and level == 1:
                        try:
                            ...
                            if property_type[0] == extract_datamodel_from_repo_url(data_model_repo_url):
                ...
                if "properties" in prop_keys:
                    partial_output = parse_yaml_dict(yaml_dict[prop]["properties"], data_model_repo_url, level + 1)
                    output = dict(output, **partial_output)
                if "items" in prop_keys and yaml_dict[prop]["items"]:
                    if isinstance(yaml_dict[prop]["items"], list):
                        for index in range(len(yaml_dict[prop]["items"])):
                            partial_output = (
                                parse_yaml_dict(yaml_dict[prop]["items"][index], data_model_repo_url, level + 1))
                            output = dict(output, **partial_output)
                    else:
                        partial_output = parse_yaml_dict(yaml_dict[prop]["items"], data_model_repo_url, level + 1)
                        output = dict(output, **partial_output)
                if "anyOf" in prop_keys:
                    partial_output = parse_yaml_dict(yaml_dict[prop]["anyOf"], data_model_repo_url, level + 1)
                    output = dict(output, **partial_output)
                if "allOf" in prop_keys:
                    partial_output = parse_yaml_dict(yaml_dict[prop]["allOf"], data_model_repo_url, level + 1)
                    output = dict(output, **partial_output)
                if "oneOf" in prop_keys:
                    partial_output = parse_yaml_dict(yaml_dict[prop]["oneOf"], data_model_repo_url, level + 1)
                    output = dict(output, **partial_output)

    return output


def is_well_documented(output, yaml_dict, data_model_repo_url):
    """
    ...
    # output[documented] = {}
    output[documented] = parse_yaml_dict(yaml_dict, data_model_repo_url, 1)
